...
CRATELIBC_PV="0.2.21"
CRATELIBC_PKG="libc-${CRATELIBC_PV}.crate"
CRATELIBC_HASH="88ee81885f9f04bff991e306fea7c1c60a5f0f9e409e99f6b40e3311a3363135"
CRATELIBC_URI="https://crates.io/api/v1/crates/libc/${CRATELIBC_PV}/download"
CRATELIBC_STAMP=".stamp_fetched-crate_libc-${CRATELIBC_PKG}.hash"
...
src_prepare() {
	...
	echo "${CRATELIBC_HASH} ${CRATELIBC_PKG}" > "${STAMP_DIR}/${CRATELIBC_STAMP}" || die
	echo "${RUSTZCASH_HASH} ${RUSTZCASH_PKG}" > "${STAMP_DIR}/${RUSTZCASH_STAMP}" || die

	# Symlink dependencies
	ln -s "${DISTDIR}"/${BDB_PKG} "${DEP_SRC}" || die
	ln -s "${DISTDIR}"/${CRATELIBC_PKG} "${DEP_SRC}" || die
