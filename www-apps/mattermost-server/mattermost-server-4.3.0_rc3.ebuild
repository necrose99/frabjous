...
src_install() {
	...
	newins config/default.json config.json.example
	fowners mattermost:mattermost /etc/${PN}/config.json.example
	fperms 600 /etc/${PN}/config.json.example
...
pkg_postinst() {
	if [ ! -e "${EROOT%/}"/etc/${PN}/config.json ]; then
		elog "No config.json found, copying the example over"
		cp -a "${EROOT%/}"/etc/${PN}/config.json{.example,} || die
	else
		elog "config.json found, please check example file for possible changes"
	fi
}
