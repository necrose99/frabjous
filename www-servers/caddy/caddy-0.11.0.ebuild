...
SRC_URI="https://${EGO_PN}/archive/v${PV}.tar.gz -> ${P}.tar.gz
	...
	geoip? (
		${HTTP_GEOIP1_URI} -> ${HTTP_GEOIP1_P}.tar.gz
		${HTTP_GEOIP2_URI} -> ${HTTP_GEOIP2_P}.tar.gz
	)
	...
	ipfilter? (
		...
		!geoip? ( ${HTTP_GEOIP2_URI} -> ${HTTP_GEOIP2_P}.tar.gz )
	)
	jwt? (
...
SLOT="0"
KEYWORDS="~amd64"
IUSE="authz awses awslambda cache cgi cors datadog expires filter
	forwardproxy geoip git grpc ipfilter jwt locale login mailout
	minify multipass nobots prometheus proxyprotocol ratelimit realip
	reauth restic telemetry test upload webdav"
...
src_unpack() {
	...
	use geoip && EGO_VENDOR+=(
		"${HTTP_GEOIP1_EGO_PN} ${HTTP_GEOIP1_COMMIT}"
		"${HTTP_GEOIP2_EGO_PN} ${HTTP_GEOIP2_COMMIT}"
	)
...
src_prepare() {
	...
	if use geoip; then
		sed -i "/(imported)/a _ \"$HTTP_GEOIP1_EGO_PN\"" \
			${CADDYMAIN}/run.go || die
	fi
	...
	if use ipfilter; then
		sed -i "/(imported)/a _ \"$HTTP_IPFILTER_EGO_PN\"" \
			${CADDYMAIN}/run.go || die
	fi
...
src_compile() {
	...
	local mygoargs=(
		...
		-gcflags "-trimpath=${S}"
		-ldflags "-s -w
			-X ${PKG}.enableTelemetry=$(usex telemetry true false)
			-X ${PKG}.gitTag=${PV}"
		-o ${PN}_ ${EGO_PN}/${PN}
	)
