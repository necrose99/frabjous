...
src_prepare() {
	...
	eapply "${FILESDIR}"/"${P}"-upstream_bugfix.patch
...
pkg_postinst() {
	if ! use systemd ; then
		einfo
		einfo "Note: Do not change the argument '--forks=1', it's buggy."
		einfo
		einfo "You can run multiples instances by creating a"
		einfo "symbolic link from kresd to kresd.foo in /etc/init.d."
		einfo
	fi
}
